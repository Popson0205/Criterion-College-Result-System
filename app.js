// ============================================================
// CRITERION COLLEGE — MAIN APP
// Admin: admin / criterion2025
// ============================================================

// ── Router ──────────────────────────────────────────────────
const ADMIN_USER = 'admin';
const ADMIN_PASS = 'criterion2025';
...
function isLoggedIn() { return sessionStorage.getItem('cc_auth') === '1'; }
function login(u,p) {
  if (u===ADMIN_USER && p===ADMIN_PASS) { sessionStorage.setItem('cc_auth','1'); return true; }
  return false;
}
function logout() { sessionStorage.removeItem('cc_auth'); navigate('login'); }
...
function checkShareToken() {
  ...
  if (token) {
    const t = DB.getByToken(token);
    if (t) {
      previewStudentId = t.studentId;
...
function render() {
  ...
  if (!isLoggedIn()) { app.innerHTML = renderLogin(); attachLoginEvents(); return; }
  app.innerHTML = `
    <div class="layout">
      ${renderSidebar()}
      <div class="main-area">
  ...
  <div class="login-screen">
    <div class="login-card card">
      ...
      <div class="form-group">
        ...
        <input id="l-user" type="text" class="input" placeholder="admin" autocomplete="username" />
      </div>
      <div class="form-group" style="margin-top:12px;">
        <label>Password</label>
        <input id="l-pass" type="password" class="input" placeholder="••••••••" autocomplete="current-password" />
      </div>
      <button class="btn btn-primary" style="width:100%;margin-top:20px;" onclick="doLogin()">
        Sign In
      </button>
      <div style="text-align:center;margin-top:12px;font-size:12px;color:var(--text-muted);">
        Default: admin / criterion2025
      </div>
    </div>
  </div>`;
}
...
function doLogin() {
  ...
  if (login(u,p)) { navigate('dashboard'); }
  else {
...
function renderSidebar() {
  const nav = [
    ...
    { id:'batch_print', icon:'🖨️',  label:'Batch Print' },
...
function renderPage() {
  switch(currentPage) {
    ...
    case 'add_student': return renderAddStudent();
    case 'edit_student': return renderAddStudent(editStudentId);
    case 'results':   return renderResults();
    case 'enter_result': return renderEnterResult();
    case 'preview_result': return renderPreviewResult();
    case 'batch_print': return renderBatchPrint();
...
function renderPublicResult() {
  ...
  const token  = params.get('share');
  const t = DB.getByToken(token);
  if (!t) return `<div style="text-align:center;padding:60px;font-family:Arial;"><h2>Invalid or expired link.</h2></div>`;
  const student = DB.getStudent(t.studentId);
  const result  = DB.getResult(t.studentId, t.session, t.term);
...
// ── RESULT ACTIONS ────────────────────────────────────────────
function saveResult(andPreview=false) {
  const settings = DB.getSettings();
  const student  = DB.getStudent(editStudentId);
  if (!student) return;
  const subjects = CLASS_SUBJECTS[student.classId]||[];
...
function shareResult(studentId) {
  const settings = DB.getSettings();
  const token = DB.createShareToken(studentId, settings.session, settings.term);
...
function saveCrecheResult(andPreview=false) {
  const settings = DB.getSettings();
  const student  = DB.getStudent(editStudentId);
  if (!student) return;
